...
public class OrderService {
    ...
    public Page getSellmanOrderPage(int status,int pageNumber, int typeId) {
        Page p = new Page();
        p.setPageNumber(pageNumber);
        int pageSize = 10;
        int totalCount = 0;
        try {
            List<Integer> list_order = oDao.selectSellmanOrdersId(typeId);
            totalCount = oDao.getSellmanOrderCount(status, list_order);
        } catch (SQLException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
        ...
        List list=null;
        try {
            List<Integer> list_orderId = oDao.selectSellmanType(typeId);
            list = oDao.selectSellmanOrderList(status, pageNumber, pageSize, list_orderId);
            for(Order o :(List<Order>)list) {
                List<OrderItem> l = oDao.selectAllItem(o.getId());
                o.setItemList(l);
            }
    ...
    public List<Integer> getAmountListByTypeID(int type_id){
        List<Integer> list=null;
        try {
            list = oDao.getAmountListByTypeID(type_id);
        } catch (SQLException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
    ...
    public List<Integer> getOrderListByTypeID(int type_id){
        List<Integer> list=null;
        try {
            list = oDao.getOrderListByTypeID(type_id);
        } catch (SQLException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
    ...
    public List<String> getDatetimetListByOrderIDs(List<Integer>OrderIDs){
        List<String> list = new ArrayList<>();
        try {
            for(Integer i : OrderIDs){
                list.add(oDao.getDateTimeByOrderID(i));
            }
        } catch (SQLException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
